...
class BertExampleTest(tf.test.TestCase):
  ...
  def test_building_example_for_meaning(self):
    ...
    self.assertEqual(example.features['input_ids_source'],
                     [0, 3, 4, 5, 1, 2, 2, 2])
    self.assertEqual(example.features['input_mask_source'],
                     [1, 1, 1, 1, 1, 0, 0, 0])
    self.assertEqual(example.features['segment_ids_source'],
                     [0, 0, 0, 0, 0, 0, 0, 0])
    self.assertEqual(example.features['input_ids_summary'],
                     [0, 3, 4, 1, 2, 2, 2, 2])
    self.assertEqual(example.features['input_mask_summary'],
                     [1, 1, 1, 1, 0, 0, 0, 0])
    self.assertEqual(example.features['segment_ids_summary'],
                     [0, 0, 0, 0, 0, 0, 0, 0])
  ...
  def test_invaid_meaning_example(self):
    with self.assertRaises(ValueError):
      ...
      bert_example.BertExampleMeaning([0] * 2, [0], [0], [0], [0], [0], [0],
                                      [0], [0])
