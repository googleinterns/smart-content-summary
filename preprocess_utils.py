...
def calculate_stats(sentences, summaries):
    """Calculate relevant statistics for the input sentences and their corresponding 
        summaries.
    ...
    print("Average word count of original sentence is", "{:.2f}".format(np.mean(count_words)), "( std:", "{:.2f}".format(np.std(count_words)) ,")")
    ...
    count_words = np.array(count_words)
    print("Average word count of shortened sentence is", "{:.2f}".format(np.mean(count_words)), "( std:", "{:.2f}".format(np.std(count_words)) ,")")
    ...
    for i in range(len(sentences)):
        ...
        count_sentences.append(np.max([tokens_sentence.count(".") + tokens_sentence.count("!") + tokens_sentence.count("?"), 1]))
    count_sentences = np.array(count_sentences)
    count_diff = np.array(count_diff)
    print("On average, there are", "{:.2f}".format(np.mean(count_sentences)), "sentences in each original text", "( std:", "{:.2f}".format(np.std(count_sentences)), ")")
    print("On average, there are", "{:.2f}".format(np.mean(count_diff)),"words in each shortened sentence that are not in the original sentence.", \
    ...
    print("The average compression ratio is", "{:.2f}".format(np.mean(compression_ratio)), "( std:", "{:.2f}".format(np.std(compression_ratio)), ")")
...
def split_dataset(train_path, tune_path, valid_path, preprocessed_path, num_of_tuning_sam, num_of_valid_sam, whether_shuffle):
    sentences = []
    summaries = []
    ...
    if num_of_tuning_sam + num_of_valid_sam > len(sentences):
        raise Exception("The number of tuning and validation samples together exceeds the total sample size of " + str(len(sentences)))
