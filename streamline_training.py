...
def export_lasertagger_config_to_json(output_dir: Text, bert_type: Text,
                                      t2t: bool, number_of_layer: int,
                                      hidden_size: int, attention_heads: int,
                                      filter_size: int, full_attention: bool):
...
def __validate_folders(args):
  ...
  if not args.train and not args.export:
    if os.path.isdir(output_dir):
      raise Exception(
          output_dir +
          " is an existing folder. Please either delete it or rename it.")
...
def __set_parameters(args):
  """ Set global variables based on the args.
    ...
    Raises:
      ValueError when verb deletion is invalid
    """
  if args.embedding_type not in ["Normal", "POS", "Sentence", "POS_concise"]:
    raise ValueError(
        "Embedding_type must be Normal, POS, POS_concise, or Sentence")

  if args.verb_deletion_loss != 0 and args.embedding_type not in [
      "POS", "POS_concise"
  ]:
    raise ValueError(
        "Verb deletion loss is non-zero and the embedding type is not POS.")
...
def __preprocess(args):
  ...
  subprocess.call(
      ("python phrase_vocabulary_optimization.py --input_file=" +
       training_file + " --input_format=wikisplit" + " --vocabulary_size=" +
       vocab_size + " --max_input_examples=" + max_input_examples +
       " --output_file=" + output_file).split(),
      cwd=lasertagger_dir)

  print("------ Running preprocessing ------")
  tuning_preprocess_command = (
      "python preprocess_main.py --input_file=" + tuning_file +
      " --input_format=wikisplit" + " --output_tfrecord=" + output_dir +
      "/tune.tf_record" + " --label_map_file=" + output_file +
  ...
  subprocess.call(tuning_preprocess_command.split(), cwd=lasertagger_dir)

  training_preprocess_command = (
      "python preprocess_main.py --input_file=" + training_file +
      " --input_format=wikisplit" + " --output_tfrecord=" + output_dir +
      "/train.tf_record" + " --label_map_file=" + output_file +
      " --vocab_file=" + bert_dir + "/vocab.txt" +
  ...
  subprocess.call(training_preprocess_command.split(), cwd=lasertagger_dir)
...
def __training(args):
  ...
  export_lasertagger_config_to_json(output_dir, bert_type, args.t2t,
                                    args.number_layer, args.hidden_size,
                                    args.num_attention_head, args.filter_size,
                                    args.full_attention)
  ...
  if args.use_tpu:
    ...
    subprocess.call(("gsutil -m cp -r " + output_dir + "/label_map.txt " +
                     folder_in_bucket + "/").split(),
                    cwd=os.path.expanduser("~"))
    subprocess.call(("gsutil -m cp -r " + config_file_path + " " +
                     folder_in_bucket + "/").split(),
                    cwd=os.path.expanduser("~"))
    subprocess.call(("gsutil -m cp -r " + bert_dir + "/* " + folder_in_bucket +
                     "/bert/").split(),
                    cwd=os.path.expanduser("~"))
  ...
  if args.use_tpu:
    subprocess.call(("gsutil -m cp -r " + folder_in_bucket + "/model " +
                     output_dir).split(),
                    cwd=os.path.expanduser("~"))
  ...
  subprocess.call(
      ("python run_lasertagger.py" + " --label_map_file=" + output_dir +
       "/label_map.txt" + " --model_config_file=" + config_file_path +
       " --output_dir=" + output_dir + "/model" + " --do_export=true" +
       " --export_path=" + output_dir + "/export").split(),
      cwd=lasertagger_dir)
  os.rename(output_dir + "/export/" + os.listdir(output_dir + "/export")[0],
            output_dir + "/export/" + "export_model")
...
def __export_to_bucket():
  ...
  folder_name = output_dir.split("/")[-1]
  subprocess.call(("gsutil -m cp -r " + output_dir + "/export/export_model " +
                   GCP_BUCKET + folder_name + "/").split(),
                  cwd=os.path.expanduser("~"))
  subprocess.call(("gsutil -m cp -r " + output_dir + "/label_map.txt " +
                   GCP_BUCKET + folder_name + "/").split(),
                  cwd=os.path.expanduser("~"))
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      "embedding_type",
      help=
      "type of embedding. Must be one of [Normal, POS, POS_concise, Sentence]. "
  ...
  parser.add_argument(
      "-learning_rate",
      type=float,
      help="The initial learning rate for Adam. default = 3e-5",
      default=3e-5)
  parser.add_argument(
      "-num_train_epochs",
      type=int,
  ...
  parser.add_argument("-train",
                      action="store_true",
                      help="If added, skip preprocessing and start training")
  parser.add_argument(
      "-export",
      action="store_true",
      help=
      "If added, skip preprocessing and training, and start exporting to bucket"
  )
  parser.add_argument("-use_tpu",
                      action="store_true",
                      help="If added, will use tpu for training")
  parser.add_argument(
      "-gbucket",
      help="The gcp bucket where cloud TPU will store intermediary outputs to")

  parser.add_argument(
      "-t2t",
      type=bool,
      default=True,
      help="If True, use autoregressive version of LaserTagger. If false, "
      "use, feed-forward version of LaserTagger. Default is True.")
  parser.add_argument(
      "-number_layer",
      type=int,
      default=1,
      help="Number of hidden layers in the decoder. default = 1")
  parser.add_argument(
      "-hidden_size",
      type=int,
      default=768,
      help="The size of the hidden layer size in the decoder. default=768")
  parser.add_argument(
      "-num_attention_head",
      type=int,
      default=4,
      help="The number of attention heads in the decoder. default=4")
  parser.add_argument(
      "-filter_size",
      type=int,
      default=3072,
      help="The size of the filter in the decoder. default=3072")
  parser.add_argument(
      "-full_attention",
      type=bool,
      default=False,
      help="Whether to use full attention in the decoder. default=false")
  ...
  parser.add_argument(
      "-verb_deletion_loss",
      type=float,
      help="The weight of verb deletion loss. Need to be >= 0. default=0."
      "Cannot be set to a number other than 0 unless the embedding_type is POS or POS_concise.",
      default=0)

  parser.add_argument("-add_tag_loss_weight",
                      type=float,
                      help="The weight of loss for adding tags. default=1",
                      default=1)
  parser.add_argument("-delete_tag_loss_weight",
                      type=float,
                      help="The weight of loss for deleting tags. default=1",
                      default=1)
  parser.add_argument("-keep_tag_loss_weight",
                      type=float,
                      help="The weight of loss for keeping tags. default=1",
                      default=1)
  ...
  if args.export:
    ...
    subprocess.call(
        ['mkdir', os.path.expanduser(args.model_output_dir)],
        cwd=os.path.expanduser('~'))
